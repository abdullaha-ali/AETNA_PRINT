CREATE DEFINER=`asharma`@`%` PROCEDURE `SnapGroupReportsBy5Dates_FSI`()
BEGIN
/*
-----------UPDATE REPORT QUERY------------
*/
...
DECLARE mcol INT; 
  ...
  DECLARE curs CURSOR FOR  
	...
	distinct	r.`pid`,
		...
		ON r.`sfid` = m.`id`
		LEFT JOIN `event_by_day_group_FSI` e
		ON r.`sfid` = e.`id`
...
truncate `report_by_id_group_2017`;
	...
	open curs;
	curs_loop: loop
	fetch curs into pid, sfid,report_date,geid,count,Publication, zipcode, County, State;
	if bDone = 1 then 
	close curs;
	leave curs_loop;
	end if;
		...
		block2:begin
			...
			DECLARE cur2 cursor for
				...
				-- cur2 content
					...
					value (pid, sfid, report_date, gid, replace(gdate, ',', ', '), gtime,Publication, zipcode, County, State,null);
						...
						while(count2 > 0) do
							...
							-- insert into report table
							insert into `report_by_id_group_2017`
							value (pid, sfid, report_date, gid2, replace(gdate2, ',', ', '), gtime,Publication, zipcode, County, State,null);
