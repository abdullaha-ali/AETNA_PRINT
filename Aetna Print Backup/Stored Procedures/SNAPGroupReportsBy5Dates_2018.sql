...
DECLARE mcol INT; 
  ...
  DECLARE curs CURSOR FOR  
	...
	-- 	GROUP BY r.`pid`, r.`date`, e.`detail`;
		GROUP BY  e.`Venue_State`, e.`detail`;
...
truncate `report_by_id_group_2018`;
	...
	open curs;
	curs_loop: loop
	fetch curs into sfid,report_date,geid,count, County, State, Market;
	if bDone = 1 then 
	close curs;
	leave curs_loop;
	end if;
	...
	-- second loop
		block2:begin
			...
			DECLARE cur2 cursor for
				...
				GROUP BY e.`Event_Date`, e.`Venue_County`, e.`Venue_State`
				) t1
				GROUP BY t1.`time` ;
				...
				-- cur2 content
					...
					value (sfid, report_date, gid, replace(gdate, ',', ', '), lower(gtime), County, State, Market,null);
                        ...
                        LIMIT tempNum, 5
							...
							-- insert into report table
							insert into `report_by_id_group_2018`
							value (sfid, report_date, gid2, replace(gdate2, ',', ', '), lower(gtime), County, State, Market,null);
