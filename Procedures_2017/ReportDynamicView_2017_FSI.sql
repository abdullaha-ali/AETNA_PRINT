CREATE DEFINER=`root`@`%` PROCEDURE `ReportDynamicView_2017_FSI`()
BEGIN
DECLARE col  INT;
SET @dv = 'DROP VIEW IF EXISTS Aetna_2017_new.`Report_in_publication_GroupAfter_dev`';
SET @str =  "CREATE VIEW Aetna_2017_new.`Report_in_publication_GroupAfter_dev` as SELECT distinct
	`r`.`pid`               AS `pid`,
	`r`.date as pubDate,
	`r`.grouprank,
	...
	group_concat(distinct(`r`.`County`) order by `r`.`County` ) as `County`,
	group_concat(distinct(`r`.`State`) order by `r`.`State` ) as `State`,
...
ON ((`r`.`pid` = `p`.`id`)))
LEFT JOIN `Aetna_2017_new`.`event_by_day_group_FSI` `e`
ON ((`r`.`eid` = `e`.`id`)))
LEFT JOIN `Aetna_2017_new`.`publication_by_day_2017` `d`
ON (((`r`.`pid` = `d`.`pid`)
AND (`d`.`date` = `r`.`date`))))
-- where p.`Rank` <= 3
