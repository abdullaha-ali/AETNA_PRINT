CREATE DEFINER=`root`@`%` PROCEDURE `loadSNAPbygroup_2017`(
	IN `date_range` INT
...
-- select * from `event_by_day_2017_group`;

TRUNCATE `event_day1_2017`;
INSERT INTO `event_day1_2017`
SELECT
...
TRUNCATE `event_day2_2017`;
INSERT INTO `event_day2_2017`
SELECT
...
TRUNCATE `event_day3_2017`;
INSERT INTO `event_day3_2017`
SELECT
CASE 
...
FROM `qa3_2017` `d`
GROUP BY `d`.`eid`,`d`.`pid`,`d`.`ad_date`;
TRUNCATE `event_day4_2017`;
INSERT INTO `event_day4_2017`
SELECT
  `d`.`newpaper_date`  AS `insertion_date`,
  `d`.`eid`              AS `eid`,
  `d`.`pid`              AS `pid`,
  `d`.`Event_Date`       AS `Event_Date`,
  `d`.`Venue_County`     AS `Venue_County`,
...
FROM (((`Aetna_2017_new`.`report_by_id_onepromotion_2017` `r`
LEFT JOIN `Aetna_2017_new`.`aetna_print_publication` `p`
ON ((`r`.`pid` = `p`.`id`)))
...
-- CALL `SNAPGroupReportBy5Dates_2017`();
