CREATE DEFINER=`aali`@`%` PROCEDURE `loadNewEvents_FSI`(
	IN lead_brand VARCHAR(50),
    IN lang VARCHAR(1),
    OUT presentation VARCHAR(10)
...
SET @dv = 'DROP VIEW IF EXISTS AetnaPrint_2018.`event_latest_W_venueID_2018`';
SET @stmt = "CREATE 
    ALGORITHM = UNDEFINED 
    DEFINER = `aali`@`%` 
