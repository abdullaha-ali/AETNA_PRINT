CREATE DEFINER=`asharma`@`%` PROCEDURE `SNAPGroupReportsBy5Dates_Publication`()
BEGIN
/*
-----------UPDATE REPORT QUERY------------
*/
...
DECLARE mcol INT; 
  ...
  DECLARE curs CURSOR FOR  
	...
	distinct	r.`pid`,
		...
		e.Venue_State, 
		e.Market
		FROM `report_by_id_onepromotion_2017` r
		LEFT JOIN `aetna_print_publication_2017` p
		ON r.`pid` = p.`id`
		LEFT JOIN `multipleEvent_2017` m
		ON r.`sfid` = m.`id`
		LEFT JOIN `event_by_day_group_2017` e
		ON r.`sfid` = e.`id`
		LEFT JOIN `event_latest_2017` el
		ON m.`geid` = el.`id`
	-- 	GROUP BY r.`pid`, r.`date`, e.`detail`;
		GROUP BY p.`Newspaper_Name`, p.`State`, e.`detail`;

  DECLARE CONTINUE HANDLER FOR NOT FOUND SET bDone := 1;

truncate `report_by_id_group_2017`;

	open curs;
	curs_loop: loop
	fetch curs into pid, sfid,report_date,geid,count,Publication, zipcode, County, State, Market;
	if bDone = 1 then 
	close curs;
	leave curs_loop;
	end if;
	...
	-- second loop
		block2:begin
			...
			DECLARE cur2 cursor for
				...
				GROUP BY e.`Event_Date`, e.`Venue_Zip_Code`
				) t1
				GROUP BY t1.`time` ;
				...
				-- cur2 content
					...
					insert into `report_by_id_group_2017`
					value (pid, sfid, report_date, gid, replace(gdate, ',', ', '), lower(gtime),Publication, zipcode, County, State, Market,null);
                        ...
                        LIMIT tempNum, 5
							...
							-- insert into report table
							insert into `report_by_id_group_2017`
							value (pid, sfid, report_date, gid2, replace(gdate2, ',', ', '), lower(gtime),Publication, zipcode, County, State, Market,null);
